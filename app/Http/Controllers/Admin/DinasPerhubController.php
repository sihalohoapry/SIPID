...
class DinasPerhubController extends Controller
{
    ...
    // all
    public function allDishubHasil(){
        if (request()->ajax()) {
            $query = HasilPembangunan::query()->where('dinas', "perhubungan");
    ...
    public function allDishub(){
        if (request()->ajax()) {
            $query = ProgresKegiatan::query()->where('dinas', "perhubungan");
